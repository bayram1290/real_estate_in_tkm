    ...
    <div class="prod_elite_kv propObj {{$hide ? 'hide' : ''}}">
        ...
        <div class="form-group row">
            ...
            <div class="col-lg-8 col-md-9 col-sm-8 norm_g fly_mob1">
                <input type="number" min="1" class="form-control white_form" id="pkv_elite_tot_area" name="tot_area" step="0.01" value="{{ $property->area }}" required />
        ...
        <div class="form-group row">
            ...
            <div class="col-lg-8 col-md-9 col-sm-8 kuck_g">
                <input type="number" min="1" max="99" class="form-control white_form" name="floor" id="pkv_elite_floor" value="{{ $property->floor }}" required/>
        ...
        <div class="form-group row">
            ...
            <div class="col-lg-8 col-md-9 col-sm-8 kuck_g">
                <input type="number" min="2" max="100" class="form-control white_form" name="tot_floor" id="pkv_elite_tot_floor" value="{{ $property->floors_in_home }}" required />
        ...
        <div class="form-group row">
            ...
            <div class="col-lg-8 col-md-9 col-sm-8 norm_g fly_mob1">
                <input type="number" min="1" max="200" class="form-control white_form" name="resid" id="pkv_elite_resid" step="0.01" value="{{ $property->living > 0 ? $property->living : ''}}" />
            </div>
            <span class="cSpan fly_mob2">{{__('messages.meter')}}<sup>2</sup></span>
        </div>
        <div class="form-group row">
            ...
            <div class="col-lg-8 col-md-9 col-sm-8 norm_g fly_mob1">
                <input type="number" min="1" max="200" class="form-control white_form" name="kitchen" id="pkv_elite_kitchen" step="0.01" value="{{ $property->kitchen_area > 0 ? $property->kitchen_area : '' }}" />
            </div>
            <span class="cSpan fly_mob2">{{__('messages.meter')}}<sup>2</sup></span>
        </div>
        <div class="form-group row">
            ...
            <div class="col-lg-8 col-md-9 col-sm-8 kuck_g fly_mob1">
                <input type="number" min="0.3" max="10" class="form-control white_form" name="ceil" id="pkv_elite_ceil" step="0.01" value="{{$property->ceil_height > 0 ? $property->ceil_height : ''}}" />
            </div>
        ...
        <div class="form-group row">
            ...
            <div class="col-lg-8 col-md-9 col-sm-8 ubuyk_g">
                <input type="text" max="100" name="n_settlement" id="pkv_elite_name" class="form-control white_form" value="{{$property->village_name}}">
            </div>
        </div>
        <div class="form-group row">
            <label for="pkv_elite_const_year" class="col-lg-4 col-md-3 col-sm-4 form-label1">{{__('messages.const_year')}}</label>
            <div class="col-lg-8 col-md-9 col-sm-8 kuck_g">
                <input type="number" class="form-control white_form" id="pkv_elite_const_year" name="const_year" min="1900" max="{{ now()->year }}" value="{{$property->construction_year > 0 ? $property->construction_year : '' }}" />
            </div>
        </div>        
        <div class="form-group row">
        ...
        <div class="form-group row">
            ...
            <div class="btn-group toGGle col-lg-8 col-md-9 col-sm-8 dec_btns2" id="pkv_elite_pass_elevs" data-toggle="buttons">
                @for( $a = 0; $a < 5; $a++ )
                    <label class="btn btn-primary {{$a==0 ? 'cusBorRad1': ''}}{{$a==4 ? 'cusBorRad2': ''}} {{$a==$property->passenger_elevator ? 'active': '' }}">
                        <input type="radio" name="pass_elevs" value="{{$a}}" {{ $a == $property->passenger_elevator ? 'checked=checked' : ''}}>
                        {{$a==0 ? __('messages.no') : $a}}</label>    
                @endfor
            </div>
        </div>
        <div class="form-group row">
            ...
            <div class="btn-group toGGle col-lg-8 col-md-9 col-sm-8 dec_btns2" id="pkv_elite_freight_elevs" data-toggle="buttons">
                @for ($b = 0; $b < 5; $b++)
                    <label class="btn btn-primary {{$b==0 ? 'cusBorRad1': ''}}{{$b==4 ? 'cusBorRad2': ''}} {{ $b == $property->service_elevator ? 'active': '' }}">
                        <input type="radio" name="freight_elevs" value="{{$b}}" {{ $b == $property->service_elevator ? 'checked=checked' : ''}}>
                        {{$b==0 ? __('messages.no') : $b}}</label>
                @endfor                
            </div>
        </div>        
        <div class="form-group row">
            ...
            <div class="btn-group toGGle col-lg-8 col-md-9 col-sm-8 dec_btns" id="pkv_elite_parking_wrap" data-toggle="buttons">
                @foreach($parkings as $parking)
                    @if($parking->id != 3 && $parking->id != 4)
                        <label id="elite_parking" class="btn btn-primary {{$parking->id == 1 ? 'cusBorRad1' : ''}} {{$parking->id == 5 ? 'cusBorRad2' : ''}} {{ $parking->id == $property->parking_id  ? 'active' : ''}}">
                            <input type="radio" name="parking" value="{{$parking->id}}" {{$parking->id == 5 ? 'onchange=showInput()' :'onchange=hideInput()'}} {{ $parking->id == $property->parking_id ? 'checked=checked' : '' }}>@if(Lang::locale() == 'ru') {{$parking->parking_ru}}
        ...
        <div class="form-group row">
            ...
            <div class="col-lg-8 col-md-9 col-sm-8 buyk_g">
                <input type="number" min="1" max="1000" class="form-control white_form" name="place_num" id="elite_place_num" value="{{$property->parking_spots > 0 ? $property->parking_spots : ''}}">
            </div>
        </div>
        <div class="form-group row" id="ex_parking_spots_elite_parking" style="display: {{ $property->parking_id == 5 ? '' : 'none' }}">
            ...
            <div class="col-lg-8 col-md-9 col-sm-8 buyk_g">
                <input type="number" min="1" max="1000" class="form-control white_form" name="place_num_ex" id="elite_place_num_ex" value="{{$property->parking_spots_ex > 0 ? $property->parking_spots_ex : ''}}" />
            </div>
        </div>
        <br>
        <div class="form-group row">
            ...
            <div class="btn-group toGGle col-lg-8 col-md-9 col-sm-8" id="pkv_elite_pool" data-toggle="buttons">
                <label class="btn btn-primary cusBorRad1 {{ $property->pool_in_building == 1 ? 'active' : ''}}"><input type="radio" name="pool" value="1" {{ $property->pool_in_building == 1 ? 'checked=checked' : ''}}>{{__('messages.outdoor')}}</label>
                <label class="btn btn-primary cusBorRad2 {{ $property->pool_in_building == 2 ? 'active' : ''}}"><input type="radio" name="pool" value="2" {{ $property->pool_in_building == 2 ? 'checked=checked' : ''}}>{{__('messages.indoor')}}</label>
            </div>
        </div><br>
        <hr>
        ...
        <input type="hidden" name="mainImg" id="mainImg_21" value="{{$property->img}}">
        ...
        <div class="form-group row">
            ...
            <div class="col-lg-8 col-md-9 col-sm-8 buyk_g m-r-20 m_m-r-0">
                <input type="number" min="1" class="form-control price_input" id="pkv_elite_price" name="price" step="0.1" value="{{$property->price}}" required />
        ...
        <div class="form-group row">
            ...
            <div class="btn-group toGGle col-lg-8 col-md-9 col-sm-8" id="pkv_elite_home_pur_debt" data-toggle="buttons">
                <label class="btn btn-primary cusBorRad1 {{$property->house_purchase_status == 1 ? 'active' : ''}}">
                    <input type="radio" name="home_pur_debt" value="1" {{$property->house_purchase_status == 1 ? 'checked=checked' : ''}} required>{{__('messages.redeemed')}}</label>
                <label class="btn btn-primary cusBorRad2 {{$property->house_purchase_status == 2 ? 'active' : ''}}">
                    <input type="radio" name="home_pur_debt" value="2" {{$property->house_purchase_status == 2 ? 'checked=checked' : ''}}>{{__('messages.with_debt')}}
                    <div class="hide tool_tab_max1" style="right:-22px;top:-2px" data-tooltip="{{ __('messages.form_feed19') }}" data-tooltip-position="right"></div>
                    <div class="rad_btn_err1 sel_err_invis">{{ __('messages.form_feed19') }}</div>
                </label>
            </div>
        </div>
        <div class="form-group row {{$property->house_purchase_status == 2 ? '' : 'hide'}}">
            ...
            <div class="col-lg-8 col-md-9 col-sm-8 norm_g fly_mob1">
                <input type="number" min="1" class="form-control white_form" id="pkv_elite_debt_amount" name="debt_amount" step="0.1" value="{{$property->house_purchase_debt_amount}}"/>
        ...
        <div class="form-group row">
            ...
            <div class="col-lg-8 col-md-9 col-sm-8">
                <input type="text" class="form-control white_form m-b-0" id="address_21" name="address" placeholder="{{__('messages.street_holder')}}" value="{{$property->address}}" required />
