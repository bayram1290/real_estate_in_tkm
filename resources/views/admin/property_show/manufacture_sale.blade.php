...
@if(!is_null($property->parking_id))
    @foreach($parkings as $parking)
        @if($parking->id > 2 && $parking->id < 5)
            @if(((int)$property->parking_id==$parking->id) || ((int)$property->parking_id==5))
                <div class="notice">
...
@endif
@if(count($property->add_service) > 0)
<div class="notice">
    <div class="notice-title">{{__('messages.add_service')}}</div>
    <div class="notice-text">
        @foreach($property->add_service as $service)
            <i class="fa fa-check m-r-5"></i>{{$service->service_ru}}<br>
        @endforeach
    </div>
</div>
@endif
